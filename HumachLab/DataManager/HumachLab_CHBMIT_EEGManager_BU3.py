...
class HumachLab_CHBMIT_EEGManager:
    ...
    ### Change channel names after removal or ignoring some channels
    _is_change_channel_names = False
    ...
    ###
    def __init__(self, data_directory='./', removable_channel_list=[], is_remove_duplicate_channels=False, is_change_channel_names=False):
        ...
        self._is_change_channel_names = is_change_channel_names
        return
    ...
    def _convert_specific_edf_from_file(self, data_file):
        ...
        preload = True if (self._is_change_channel_names or self._is_remove_duplicate_channels) else False

        edf_data = None
        edf_data = mne.io.read_raw_edf(data_file, preload=preload, exclude=self._removable_dummy_channels)
    ...
    def _remove_duplicate_channels(self, chans:dict):
        # print(chans)
        rev_chans = {}
        for key, value in chans.items():
            rev_chans.setdefault(value, set()).add(key)

        dup_ind = [values for key, values in rev_chans.items() if len(values) > 1]

        dup_chans = {}
        for dup_it in dup_ind:
            dup_it = list(dup_it)
            dup_it.sort(key=self._natural_keys)
            #print('&&&&', dup_it)
            cnt = 0
            for i in dup_it:
                #if cnt > 0:
                dup = chans[i] + '-{}'.format(cnt)
                chans[i] = dup
                if self._is_remove_duplicate_channels and cnt > 0:
                    del chans[i]
                    dup_chans[i] = dup
                cnt += 1

        ### Changing channel names
        if self._is_change_channel_names:
            all_chans = {}
            i = 1
            j = len(list(chans.keys()))

            for chkey, chval in chans.items():
                nwkey = 'Channel {}'.format(i)
                all_chans[nwkey] = chans[chkey]
                i += 1
                if i>j:
                    break
            chans = all_chans
            ...
            all_chans = {}
            i = j+1
            j = i+len(list(dup_chans.keys()))-1
            for chkey, chval in dup_chans.items():
                nwkey = 'Channel {}'.format(i)
                all_chans[nwkey] = dup_chans[chkey]
                i += 1
                if i==j:
                    break
            dup_chans = all_chans

        return chans, dup_chans
    ...
    def _convert_summary_from_specific_summary_file(self, summary_file):
            ...
            for line in f:
                ...
                if (line == '\n'):
                    ...
                    if (chn_ind > -1):
                        ...
                        tmp_chn, tmp_dup_chn_ = self._remove_duplicate_channels(tmp_chn)
                        channels.append(tmp_chn)
                        duplicate_chn_.append(tmp_dup_chn_)
                        tmp_chn = {}
                        tmp_dup_chn_ = {}
                        chn_ind = -1
        ...
        return (sample_rate, channels, duplicate_chn_, data)



    def _populate_patient_info_with_summary_data(self, patient_no, summary_file):
        samp_rate, chans, dup_chns, rec_info = self._convert_summary_from_specific_summary_file(summary_file)
        # print(samp_rate, '\n', chans, '\n', rec_info)
        ...
        patient.duplicate_channels_ = dup_chns
            ...
            for record_no in patient.record_numbers:
                ...
                if len(tmp_ind) > 0:
                    ...
                    if self._is_remove_duplicate_channels:
                        dropable_chns = (patient.duplicate_channels_[tmp_record.channel_id])
                    ...
                    # print('+++', patient.channels, tmp_record.channel_id)
                else:
    ...
    def get_records_info_for_patients(self, patient_no:list, record_no:list=[]):
        ...
        if str(patient_no.__class__.__name__) in ['list'] and str(record_no.__class__.__name__) in ['list']:
            if len(patient_no)>0 and len(record_no)>0 and len(patient_no)==len(record_no):
                ...
                print('Please provide items in the list for patients and same number of lists inside the list of records.')
        else:
            print('Please provide both patients and records in the form of list.')
        return result
    ...
    def get_record_info_with_seizure_for_all_patient(self, patient_list:list):
        ...
        for p in patient_list:
            # print(p.patient_no, ' ', len(p.records))
            ind = []
            rno = []
            chn = []
            for r in p.records:
                # print(r.record_no,' no s: ', r.num_seizures)
                if r.num_seizures>0:
...
# # ### Some Patient info
# #  patient_no- list of patients' numbers & record_no- list of lists of records for that patient
# # p_list = manager.get_records_info_for_patients([1, 17], [[1, 5, 9], [5, 9]])
# p_list = manager.get_records_info_for_patients([1, 12], [[4, 5], []])
# # print(p_list)
# for p in p_list:
#     print(p.__dict__)
