...
############################### This data manager is for CHB-MIT dataset ############################################
...
class HumachLab_EEGManager:
    ...
    def _get_removable_channels(self, chans:dict):
        ...
        for key, chan in chans.items():
            # print('Check: ', chan, (chan in self._removable_dummy_channels))
    ...
    def _get_record_info_for_patient(self, patient_no, record_no):
        ...
        for p in list_of_patients:
            ...
            # print('KKKKK', len(patient.channels), '\n', len(patient.removable_channels_))
    ...
    def _generate_records_for_single_patient(self, patient_no, r_no):
        ...
        if not summary_stat:
            ...
            # print('LLLLLL', len(patient.channels), '\n', len(patient.removable_channels_))

        if not patient:
            self.logger.info('No record found for patient {}'.format(patient_no))
        else:
            missing_rec_ind = []
            present_chn_ind = []
            rearrange_chn_ind = 0
            ...
            for record_no in patient.record_numbers:
                ...
                if len(tmp_ind) > 0:
                    ...
                    # print(tmp_ind, '-->', tmp_record.channel_id)
            ...
            # print('LLLLLL222', len(patient.channels), '\n', len(patient.removable_channels_))
            ...
            # print('LLLLLL333', len(patient.channels), '\n', len(patient.removable_channels_))
    ...
    def get_record_info_with_seizure_for_all_patient(self, patient_list:list):
        ...
        for p in patient_list:
            # print(p.num_records, p.record_numbers)
            re = []
            re_no = []
            for r in p.records:
                if r.num_seizures>0:
            ...
            # print('#####', p.num_records, p.record_numbers, re_no)
