...
class HumachLab_CHBMIT_EEGManager_BU2:
    ### EEG directory
    data_directory = './'
    ...
    def _generate_records_for_all_patient(self):
        ...
        for p in range(1, (self.max_patients)):
            patient = self._generate_records_for_single_patient(p, [])
            if patient:
    ...
    #  p_list- list of patients' number & r_list- list of list of records for corresponding patient
    def _generate_records_for_some_patients(self, p_list, r_list):
        self.patient_list = []
        for p in p_list:
            ind = p_list.index(p)
            patient = self._generate_records_for_single_patient(p, r_list[ind])
            if patient:
                self.patient_list.append(patient)
    ...
    def _get_record_info_for_patient(self, patient_no, record_no):
        ...
        # else:
        #     data = self._generate_records_for_some_patients(patient_no, record_no)
        # return data
    ...
    def get_records_info_for_patients(self, patient_no:list, record_no:list=[]):
        ...
        if str(patient_no.__class__.__name__) in ['list'] and str(record_no.__class__.__name__) in ['list']:
            if len(patient_no)>0 and len(record_no)>0 and len(patient_no)==len(record_no):
                ...
                print('Please provide items in the list for patients and same number of lists in the list for records.')
        else:
