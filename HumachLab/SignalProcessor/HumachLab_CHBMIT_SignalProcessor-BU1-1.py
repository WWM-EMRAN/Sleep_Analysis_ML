...
from HumachLab_CHBMIT_EEGFeatureExtractor import HumachLab_CHBMIT_EEGFeatureExtractor
from HumachLab_Utility import Humachlab_Utility


class HumachLab_CHBMIT_SignalProcessor:
...
########################################################################################################################
    ...
    def create_signals_segment(self, segment_save_location:str, patients:list, segment_duration_in_seconds:int=5, overlap_duration_in_seconds:int=0, overlap_duration_in_percentage:int=0, overlap_duration_in_samples:int=0):
        ...
        for pat in patients_copy:
            ...
            self.logger.info(f'{segment_duration_in_seconds} {pat_copy.sam_rate}')
            self.logger.info(f'Durations: {segment_duration_in_samples} {overlap_duration_in_samples}')
            # ==========================================================
    ...
    #                     feat_ext_obj = HumachLab_CHBMIT_EEGFeatureExtractor(data_frame_segment)
    ...
    def create_features_dataset_from_signals_segment(self, feature_save_location:str, patients:list, segment_duration_in_seconds:int=5, overlap_duration_in_seconds:int=0, overlap_duration_in_percentage:int=0, overlap_duration_in_samples:int=0):
        ...
        for pat in patients_copy:
            ...
            self.logger.info(f'{segment_duration_in_seconds} {samp_rate}')
            self.logger.info(f'Durations: {segment_duration_in_samples} {overlap_duration_in_samples}')
            # ==========================================================
            ...
            for rec in pat_copy.records:
                ...
                for ch in data.ch_names:
                    ...
                    while seg_st<end_time:
                        ...
                        feat_ext_obj = HumachLab_CHBMIT_EEGFeatureExtractor(self.logger, data_frame_segment, manage_exceptional_data=1, signal_frequency=samp_rate, sample_per_second=segment_duration_in_samples, filtering_enabled=True)
                        ...
                        if i in (46, 47, 48, 49, 50):
                            self.logger.info(new_feats)
                            self.logger.info(extr_feats)
                            new_feats.to_csv(f'./tst_feats_{i}.csv')
                            print(f'data to write for segment {i}')
                    ...
                    if os.path.isfile(feature_file_name):
                        ...
                        new_features_columns = extracted_features.columns.tolist()
                    ...
                    # ''' Open up to  just see features not to write in the file
                    if len(addable_columns_from_new_features)>0 or not(len(existing_csv_columns) == len(new_features_ordered_list) and len(existing_csv_columns) == sum([1 for i, j in zip(existing_csv_columns, new_features_ordered_list) if i == j])):
                        ...
                        if not(len(existing_csv_columns) == len(new_features_ordered_list) and len(existing_csv_columns) == sum([1 for i, j in zip(existing_csv_columns, new_features_ordered_list) if i == j])):
