            ...
            for rec in pat_copy.records:
                ...
                for ch in data.ch_names:
                    ...
                    if os.path.isfile(feature_file_name):
                        ...
                        print(f'====> Number of segments in edf: {saved_data.shape[0]} in feature file: {future_num_segs_from_edf} | Read again: {saved_data.shape[0] != future_num_segs_from_edf}')

                        ### Calculate again the file having partially feature extracted
                        if saved_data.shape[0] != future_num_segs_from_edf:
                            existing_csv_columns = []
                            calculate_feats_again = True
                        ### Calculate again the file having some the same columns existed
                        if saved_data.shape[1] != future_num_segs_from_edf:
                            calculate_feats_again = True
