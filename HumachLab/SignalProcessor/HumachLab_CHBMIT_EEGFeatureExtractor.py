...
class HumachLab_CHBMIT_EEGFeatureExtractor:
    ...
    def __init__(self, logger, data_frame_segment, channel_name, manage_exceptional_data=0, signal_frequency = 256, sample_per_second=1280, filtering_enabled=False, lowcut=1, highcut=48):
        self.logger = logger
        self.data_frame_segment = data_frame_segment
        self.channel_name = channel_name
        self.manage_exceptional_data = manage_exceptional_data

        self.td_statistical_features = ['maximum', 'minimum', 'mean', 'median', 'standardDeviation', 'variance', 'kurtosis',
                                        'skewness', 'numberOfZeroCrossing', 'positiveToNegativeSampleRatio', 'meanAbsoluteValue']
        # self.td_statistical_features = ['maximum', 'minimum', 'mean', 'median', 'standardDeviation', 'variance', 'kurtosis',
        #                                 'skewness', 'numberOfZeroCrossing', 'positiveToNegativeSampleRatio', 'positiveToNegativePeakRatio', 'meanAbsoluteValue']
        self.td_nonlinear_features = ['approximateEntropy', 'sampleEntropy', 'permutationEntropy', 'spectralEntropy',
                                      'singularValueDecompositionEntropy', 'fuzzyEntropy', 'distributionEntropy', 'shannonEntropy', 'renyiEntropy']
        self.td_nonlinear_features2_complexity_and_fractal_dimensions = ['lempelZivComplexity', 'hjorthMobility', 'hjorthComplexity',
                                                                         'fisherInfo', 'petrosianFd', 'katzFd', 'higuchiFd', 'detrendedFluctuation']
        # 'hurstExponent',
        ...
        self.td_nonlinear_features3_samp_entropy_profiling = ['entropyProfiled_total_sampleEntropy', 'entropyProfiled_average_sampleEntropy', 'entropyProfiled_maximum_sampleEntropy',
        ...
        self.fd_statistical_features = ['fd_maximum', 'fd_minimum', 'fd_mean', 'fd_median', 'fd_standardDeviation', 'fd_variance', 'fd_kurtosis', 'fd_skewness',
        ...
        self.fd_spectral_features2 = ['fd_bandPower', 'fd_bandPower_alpha', 'fd_bandPower_beta', 'fd_bandPower_delta', 'fd_bandPower_theta', 'fd_bandPower_other']

        self.band_frequency_list = {'alpha': (8, 14), 'beta': (14, 31), 'delta': (0, 5), 'theta': (5, 8), 'gamma': (31, 100), 'other': (31, 51)}
        self.band = (0, signal_frequency)
        # self.band_frequency_list = {'alpha': (8, 12), 'beta': (12, 30), 'delta': (0.5, 4), 'theta': (4, 8), 'gamma': (30, 100), 'other': (30, 51)}
        ...
        self.feature_list = self.td_statistical_features + self.td_nonlinear_features + self.td_nonlinear_features2_complexity_and_fractal_dimensions + \
                            self.td_nonlinear_features3_samp_entropy_profiling + self.fd_statistical_features
        # self.feature_list = self.td_statistical_features + self.td_nonlinear_features + self.td_nonlinear_features2_complexity_and_fractal_dimensions + \
        #                     self.td_nonlinear_features3_samp_entropy_profiling + self.fd_statistical_features + self.fd_spectral_features2
    ...
    def get_all_features(self, already_existing_features=[], feature_types=0):
        ...
        seg_values = self.data_frame_segment[self.channel_name].values.flatten() #np.array(self.data_frame_segment) # self.data_frame_segment.values.flatten()
        ...
        for feat in feature_names:
            ...
            try:
                ...
                if feat.startswith('fd_'):
                    if (feat in (self.fd_statistical_features)):
                    ...
                    elif (feat in (self.fd_spectral_features2)):
                        final_feat_list = (feat.split('_'))
                        fnl = len(final_feat_list)
...
############ Entropy
    ...
    # def spectralEntropy(self, data):
    #     # data = self.data_frame_segment.values.flatten()
    #     # data = np.array(self.data_frame_segment[self.channel_name]) #self.data_frame_segment[self.channel_name].values.flatten()
    #     sf = self.signal_frequency
    #     # se = spectral_entropy(data, sf)
    #     se = spectral_entropy(data, sf, method='welch')
    #     # print(f'spectralEntropy...{se}')
    #     if math.isnan(se):
    #         print(f'SpectralEntropy is {se}')
    #         print(type(data), data)
    #         # self.data_frame_segment.to_csv(f'./tst_data2.csv')
    #         # tmp_df = pd.DataFrame(data=data, columns=[self.channel_name])
    #         # tmp_df.to_csv(f'./tst_data3.csv')
    #     return se
    ...
    def fuzzyEntropy(self, data, m=2):
