...
########################################################################################################################
            ...
            for rec in pat_copy.records:
                ...
                for ch in data.ch_names:
                    ...
                    while seg_st<end_time:
                        ...
                        feat_ext_obj = HumachLab_CHBMIT_EEGFeatureExtractor(self.logger, data_frame_segment, ch, manage_exceptional_data=0, signal_frequency=samp_rate, sample_per_second=segment_duration_in_samples, filtering_enabled=True)
    ...
    def correct_feature_data(self, feature_save_location:str, patients:list, segment_duration_in_seconds:int=5, overlap_duration_in_seconds:int=0, overlap_duration_in_percentage:int=0, overlap_duration_in_samples:int=0):
        ...
        task_id = 0 #1=replace row, 2=remove row, 3=replace column, 4=remove column
        if task_id==0:
            return
