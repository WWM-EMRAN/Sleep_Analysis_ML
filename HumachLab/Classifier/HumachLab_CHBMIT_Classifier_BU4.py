...
# ### Necessary parameter setup for data management and training-testing
    ...
    def __init__(self, log_path='./', should_use_params=False, parameter_optimization=3, early_stop=False, cross_validation_rounds=5, exp_name='test', save_no='00', test_split_ratio=0.30, class_name='class', classifier_methods=[], random_training=0, should_balanced_data = False, channels=[], patients=[], records=[]):
    ...
    def create_log_path(self, path):
        ...
        path = path + f'Experiment_Classifier_{srl}/'
        if not os.path.exists(path):
            os.mkdir(path)
            
        tmp_log_path = f'{path}/temporary_log_{srl}.txt'
        self.util_obj = Humachlab_Utility(tmp_log_path)
...
# ### Loading or populating data for training
    ...
    def get_external_data(self, dat_file_name='All_Data_Left_Hemisphere.csv', replace_transitions=False, all_channels_columnwise=False):
        ...
        if os.path.isfile(all_data_csv):
            ...
            self.util_obj.print_and_store_log(df.shape)

            self.util_obj.print_and_store_log(f'Shape before: {df.shape}')
            df = df[df != np.inf]
            df = df.fillna(0)
            ...
            self.util_obj.print_and_store_log(f'Shape after: {df.shape}')
            if len(channels)>0:
            ...
            if all_channels_columnwise:
                ...
                for chn in channels:
                    self.util_obj.print_and_store_log(chn)
                    ...
                    self.util_obj.print_and_store_log(tmpdf.shape, tdf.shape)
                    if tmpdf.empty:
                ...
                df = tmpdf
            self.util_obj.print_and_store_log(f'Shape after2: {df.shape}')
        return df


    def load_data(self, path, class_name, channels=[], patients=[], records=[]):
        # patient = ':02d'.format(patient)
        self.util_obj.print_and_store_log(path, channels)
        df = pd.DataFrame()
        path2 = os.walk(path)

        for root, directories, files in path2:
            # for directory in directories:
            #     self.util_obj.print_and_store_log(directory)
            for file in files:
                #             self.util_obj.print_and_store_log(file)
                items = file.split('_')
                pat = int(items[1])
                rec = int(items[2])
                ch = (items[3].split('.'))[0]
        ...
        #     self.util_obj.print_and_store_log(df)
        # if df.shape[0] > 0:
    ...
    def set_or_load_all_data(self, path='./', feature_subdirectories='single_channel_feafures', replace_transitions=False, all_channels_columnwise=False, dataframe=None, dat_file_name=''):
        ...
        if ((dataframe is None) and (path is None) and (dat_file_name is None)):
            self.util_obj.print_and_store_log('No data is provided or instructed to retrieve...')
            return
            ...
            self.util_obj.print_and_store_log('External data is set...')
        else:
            ...
            for root, directories, files in path2:
                ...
                # self.util_obj.print_and_store_log(selected_dirs, channels)
                selected_dirs = [sdir for sdir in selected_dirs if (sdir.split('_')[1]) in channels]
                # self.util_obj.print_and_store_log(selected_dirs)
                ...
                for directory in directories:
                    ...
                    self.util_obj.print_and_store_log(df.shape, tdf.shape)
                    #             continue
        ...
        self.util_obj.print_and_store_log(f'Dataset set with shape: {self.dataset.shape}')
        self.util_obj.print_and_store_log(list(self.dataset.columns))
...
# ### Patient specific data splitting and feature-target seperator
    ...
    def _patient_specific_data_splitter(self, dat, p_id, test_split_ratio=0.0):
        ...
        if test_split_ratio == 0.0:
            ...
            self.util_obj.print_and_store_log(no_of_ts_patients, no_of_tr_patients)
            ...
            self.util_obj.print_and_store_log(ts_pat_ids, tr_pat_ids)
...
# ### Patient specific training and testing
    ...
    def patient_specific_testing(self, classifier_method='rf'):
        ...
        # self.log_path = self.create_log_path(self.log_path_orig)
        self.util_obj.print_and_store_log(f'\n##########********** Classification with: {classifier_method} **********##########\n#################################################################\n')

        data = self.dataset
        if data is None:
            self.util_obj.print_and_store_log('No data is provided, please try again after setting proper data...')
            return
        ...
        for pat_id in unique_pat_ids:
            ...
            for i in range(len(all_trained_models)):
                ...
                # self.util_obj.print_and_store_log('$$$-> ', ind, tr_mod_scr['validation_patient'])
                ...
                self.util_obj.print_and_store_log('==', model_scores)

                # # For best model
                if i==best_model_index:
                    # self.util_obj.print_and_store_log('bst mod ind =========================> ', pat_id, best_model_index)
            ...
            self.util_obj.print_and_store_log(f'### Testing ENDS for patient {pat_id}')
    ...
    def patient_specific_training(self, data, classifier_method='rf'):
        ...
        for i in index_list:
            ...
            self.util_obj.print_and_store_log(f'{i}- Training and validation STARTS for patient {pat_id}')
            ...
            # self.util_obj.print_and_store_log('********** F1', f1scr)
            ...
            self.util_obj.print_and_store_log(f'Training and validation ENDS for patient {pat_id}')
...
# ### Apply Grid-search and calculate scores
    ...
    def run_model_gridSearch(self, mods, params, training_data, test_data):
        ...
        self.util_obj.print_and_store_log(f'Best estimator of the model: {model.best_estimator_}')

        # if self.should_use_params:
        #     bst_parameters = model.best_params_
        #     self.util_obj.print_and_store_log('&&&&&&&&&&&&&&&&&&&&&&', bst_parameters)
...
# ### Apply Baysian-search and calculate scores
    ...
    def run_model_baysianSearch(self, mods, params, training_data, test_data):
        ...
        model = BayesSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, iid=False, n_jobs=-1, verbose=2)
...
#############################
    ...
    def calculate_model_scores(self, mods, y_pred, y_test):
        ...
        self.util_obj.print_and_store_log(scr_dict)
...
# ### Save all information to logs
    ...
    # # ### Temporary log
    # def print_and_store_log(self, tmp_data):
    #     print(tmp_data)
    ...
    # 
    #     PIK = f'{self.log_path}/temporary_log_{save_no}.txt'
    #     with open(PIK, 'a') as f:
    #         result = str(tmp_data)+'\n'
    #         f.write(result)
    #         print('New data is added to the temporary log file...')
    # 
    #     return
    # # ########
    ...
    def save_and_show_model_scores(self, all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs):
        ...
        PIK = f'{self.log_path}/all_models_{save_no}.dat'
        with open(PIK, 'wb') as f:
            pickle.dump(len(all_models), f)
            for i in range(len(all_models)):
                p = all_models[i]
                pickle.dump(p, f)
            self.util_obj.print_and_store_log('All models are written to the file...')

        PIK = f'{self.log_path}/bst_models_{save_no}.dat'
        with open(PIK, 'wb') as f:
            pickle.dump(len(bst_models), f)
            for i in range(len(bst_models)):
                p = bst_models[i]
                pickle.dump(p, f)
            self.util_obj.print_and_store_log('Best models are written to the file...')

        PIK = f'{self.log_path}/tr_score_{save_no}.dat'
        with open(PIK, 'wb') as f:
            pickle.dump(len(all_training_scrs), f)
            for i in range(len(all_training_scrs)):
                p = all_training_scrs[i]
                pickle.dump(p, f)
            self.util_obj.print_and_store_log('Training score is written to the file...')

        PIK = f'{self.log_path}/ts_score_{save_no}.dat'
        with open(PIK, 'wb') as f:
            pickle.dump(len(all_test_scrs), f)
            for i in range(len(all_test_scrs)):
                p = all_test_scrs[i]
                pickle.dump(p, f)
            self.util_obj.print_and_store_log('Test score is written to the file...')

        PIK = f'{self.log_path}/bst_score_{save_no}.dat'
        with open(PIK, "wb") as f:
            pickle.dump(len(bst_test_srcs), f)
            for i in range(len(bst_test_srcs)):
                p = bst_test_srcs[i]
                pickle.dump(p, f)
            self.util_obj.print_and_store_log('Best score is written to the file...')

        # ### Show model scores and details
        self.util_obj.print_and_store_log(len(bst_models), len(all_test_scrs), len(all_training_scrs), len(bst_test_srcs))
        ...
        for bst in bst_test_srcs:
            ...
            self.util_obj.print_and_store_log(f'\nRESULT {ind+1}\n')
            self.util_obj.print_and_store_log(f'##################################################################\n')
            self.util_obj.print_and_store_log(f'BEST TEST SCORES\n')
            self.util_obj.print_and_store_log(f'------------------------------------------------------------------\n')
            self.util_obj.print_and_store_log(bst)
            self.util_obj.print_and_store_log(f'==================================================================\n')
            self.util_obj.print_and_store_log(f'ALL TEST SCORES\n')
            self.util_obj.print_and_store_log(f'------------------------------------------------------------------\n')
            # for tss in all_test_scrs:
            #     self.util_obj.print_and_store_log(f'{tss}\n')
            self.util_obj.print_and_store_log(all_test_scrs[ind])
            self.util_obj.print_and_store_log(f'==================================================================\n')
            self.util_obj.print_and_store_log(f'ALL TRAINING SCORES\n')
            self.util_obj.print_and_store_log(f'------------------------------------------------------------------\n')
            # for trs in all_training_scrs:
            #     self.util_obj.print_and_store_log(f'{trs}\n')
            self.util_obj.print_and_store_log(all_training_scrs[ind])
            self.util_obj.print_and_store_log(f'==================================================================\n')
    ...
    def save_metadata_and_prediction_targets(self, classifier_method, pat_ids, y_tests, y_preds):
        ...
        PIK = f'{self.log_path}/metadata_{save_no}.txt'
        metadata_dict = {}
        metadata_dict['exp_no'] = save_no
        ...
        #should_use_params=True, cross_validation_rounds=2, exp_name=f'Test {test_no}: {test_details}', save_no=f'{test_no}', test_split_ratio=0.3, class_name=

        # ### Saving dictionary using pickle
        with open(PIK, 'wb') as f:
            pickle.dump(metadata_dict, f, protocol=pickle.HIGHEST_PROTOCOL)
            self.util_obj.print_and_store_log('Metadata is written to the file...')

        # # ### Retrieving dictionary using pickle
        # with open(PIK, 'rb') as f:
        #     dict_data = pickle.load(f)
        #     self.util_obj.print_and_store_log('Metadata is read from the file...')


        # ### Saving prediction target data
        save_file = f'{self.log_path}/bst_prediction_data_{save_no}.csv'
        ...
        for i in range(len(y_tests)):
            ...
            # self.util_obj.print_and_store_log(list(y_tst), '\n\n', list(y_prd))
        ...
        self.util_obj.print_and_store_log('Prediction result is written to the file...')
...
# ### All models' implementation
    ...
    def random_forest(self, training_data, test_data):
        ...
        if self.parameter_optimization == 1:
            ...
            model, model_scores = self.run_model_randomizedSearch(RandomForestClassifier(), parameters, training_data, test_data)
        elif self.parameter_optimization == 3:
            model, model_scores = self.run_model_baysianSearch(RandomForestClassifier(), parameters, training_data, test_data)
