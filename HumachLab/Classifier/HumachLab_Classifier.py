...
if sys_ind==0:
    ...
    from HumachLab.Classifier.HumachLab_Preprocessor import *
    from HumachLab.Utility.HumachLab_Utility import *
...
# ### Necessary parameter setup for data management and training-testing
    ...
    def __init__(self, logger, log_path='./', test_no='00', should_use_params=False, parameter_optimization=3, early_stop=False, cross_validation_rounds=5, exp_name='test', class_name='class', test_split_ratio=0.30, test_splitting_nature=0, validation_split_ratio=0.30, validation_splitting_nature=0, should_balanced_data = False, best_model_scoring_metrics=[ML_Performace_Metrics.RECL, ML_Performace_Metrics.F1SCR], channels=[], patients=[], records=[], patients_or_folds_done=0):
        ...
        self.tst_nat = f'patient-specific testing'
        self.val_nat = f'patient-specific training'

        self.dataset = pd.DataFrame()
        self.feature_type = 'All features'
        ...
        self.patients_or_folds_done = patients_or_folds_done
    ...
    def set_or_load_all_data(self, path='./', feature_subdirectories='single_channel_feafures', replace_transitions=0, drop_nan=False, all_channels_columnwise=False, dataframe=None, dat_file_name=''):
        ...
        df = pd.DataFrame()
        already_colwise = False

        if ((dataframe is None) and (path is None) and (dat_file_name is None)):
            self.logger.warning(f'No data is provided or instructed to retrieve...')
            return
            ...
            df, already_colwise = self.dataManager.load_external_data(dat_file_name, patients, records, channels, replace_transitions, drop_nan, all_channels_columnwise)
        ...
        print(all_channels_columnwise, already_colwise)
        make_columnwise = False
        if all_channels_columnwise==True and already_colwise==True:
            already_colwise = False
        elif all_channels_columnwise==True and already_colwise==False:
            already_colwise = all_channels_columnwise
            make_columnwise = all_channels_columnwise
        print(all_channels_columnwise, already_colwise)


        self.logger.info(f'Before processing: \nDataShape: {df.shape} '
                         f'\nDataColumns:{list(df.columns)} '
                         f'\nDataChannels:{self.channels if all_channels_columnwise else df[self.channel_cols[1]].unique()}')

        self.dataset = df.copy() #UNCOMMENT when dealing with new file
        self.dataset = self.dataManager.get_data_for_specific_channels(df, channels)
        # self.dataset = self.dataManager.filter_and_replace_data_in_dataframe(df, patients, records, channels, replace_transitions, drop_nan, make_columnwise)
        # self.dataset = self.dataManager.filter_and_replace_data_in_dataframe(df, patients, records, channels, replace_transitions, drop_nan, already_colwise)
        ...
        self.logger.info(f'After processing: \nDataShape: {self.dataset.shape} '
    ...
    def do_preprocessing(self, preprocessing_type, prep_feature_type=[]):
        #Preprocessing

        self.logger.info(f'### Pre-processing STARTS...')

        # Apply domain based feature group selection
        if preprocessing_type==1:
            self.logger.info(f'Select features from groups from dataset manually ...')
            # setattr(self, name, value)  # equivalent to: self.varname= 'something'
            self.dataset = self.preprocessor.get_selected_feature_groups_from_dataframe(self.dataset, self.class_name, self.channel_cols[1], prep_feature_type=prep_feature_type)
            self.feature_type = ', '.join(prep_feature_type)
        # Apply arbitrary feature selection
        if preprocessing_type==2:
            self.logger.info(f'Select features from dataset manually ...')
            # setattr(self, name, value)  # equivalent to: self.varname= 'something'
            self.dataset = self.preprocessor.get_selected_features_from_dataframe(self.dataset, self.class_name, self.channel_cols[1], prep_feature_name=prep_feature_type)
            self.feature_type = ', '.join(prep_feature_type)
        # Apply PCA
        elif preprocessing_type==8:
            self.logger.info(f'Apply PCA ...')
            self.dataset, _, _ = self.preprocessor.get_PCA_from_dataframe(self.dataset, self.class_name, num_comp=0, allowed_variance=0.99)
            self.feature_type = 'PCA features'
        else:
            self.feature_type = 'All features'
            self.logger.info(f'No preprocessing type is selected.')

        # self.dataset = self.preprocessor.get_resamplled_data(self.dataset, self.class_name, self.pat_id_col, random_sampling=True, up_or_down_sampling=1, min_scale=2.0, max_scale=3.0)

        col_lst = list(self.dataset.columns)
        self.logger.info(f'DataShape: {self.dataset.shape} '
                         f'\nDataColumns:{col_lst} '
                         f'\nDataChannels:{HumachLab_StaticMethods.get_dataset_channel_names(self.dataset, self.logger)}')

        self.logger.info(f'### Pre-processing ENDS...')
    ...
    def do_classification(self, classification_type, clf_meth, num_fold, is_train_will_all_data):
        #Classification
        if classification_type == 1:
            self.patient_specific_testing(classifier_method=clf_meth, num_fold=num_fold, is_train_will_all_data=is_train_will_all_data)
        elif classification_type == 2:
            self.cross_patient_training_and_patient_specific_testing(classifier_method=clf_meth, num_fold=num_fold)
        elif classification_type == 3:
            self.cross_patient_training_and_testing(classifier_method=clf_meth, num_fold=num_fold, is_train_will_all_data=is_train_will_all_data)
        else:
    ...
    def _get_data_from_patient_or_serial_indices(self, indices, force_patient_spesific=False, from_training=False):
        data = copy.deepcopy(self.dataset)
        ...
        if from_training:
            # self.logger.info(f'Adding additional overlapped data...')
            # dat_file_name = 'E:/aliem/AllData/CHBMIT_SeizOnly_S5O4_ch22_pat24_rec136_ft92.csv'
            # all_channels_columnwise = False if self.channel_cols[1] in all_cols else True
            # ovr_data, already_colwise = self.dataManager.load_external_data(dat_file_name, [], [], [], 1, False, all_channels_columnwise)
            # ovr_data = self.dataManager.filter_and_replace_data_in_dataframe(ovr_data, indices, [[] for _ in range(len(indices))], [], 1, False, (not all_channels_columnwise))
            # self.logger.info(f'Data shape after resampling: {ovr_data.shape}')
            # data = pd.concat([data, ovr_data])
        ...
        self.logger.info(f'================> All Data, {from_training}, {data.columns.values.tolist()}')
        self.logger.info(f'================> All Data, {indices}, {data.shape}')
        if self.is_patient_spesific_analysis or force_patient_spesific:
            data = data[data[self.pat_id_col].isin(indices)]
        else:
            data = data[data[self.serial_col].isin(indices)]

        # self.logger.info(f'pat_ids--> {data["pat_id"].unique()}')
        target = data[self.class_name].values.tolist()

        extra_cols = data[ex_col]
        # extra_cols = extra_cols.values
        self.logger.info(f'================> Extra: {extra_cols.shape}')
        features = data.drop(ex_col, axis=1)
        features = features.drop([self.class_name], axis=1)
        self.logger.info(f'================> Features: {features.shape}')
        features = features.values
        del data
        # print('TTTTTTTTT2222222222', type(features), type(extra_cols))
        # print('TTTTTTTTT3333333333', features.columns.values.tolist(), extra_cols.columns.values.tolist())
        print('XXXXXXXXXXXXXXXXX: ', np.where(np.isnan(features)))
        return features, target, extra_cols
    ...
    def patient_specific_testing(self, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False):
        self.is_patient_spesific_analysis = True
        self.exp_type = f'patient-wise training and testing'
        ...
        all_models, all_best_model, all_models_training_scores, all_models_test_scores, all_best_test_scores, all_extra, all_y_tests, all_y_preds, self.best_model_indices, splitter_b = self.load_partial_models_scores_prediction_and_metadata()

        splitter = None
        spl_rand = 312 ##random.randint(1, 1000)
        self.logger.info(f'Splitter random value: {spl_rand}')
        if self.test_splitting_nature > 0:
            # Random splitting
            split_num = self.test_splitting_nature
            # split_num = split_num if (split_num<=len(unique_pat_ids)) else len(unique_pat_ids)
            splitter = ShuffleSplit(n_splits=split_num, test_size=self.test_split_ratio, random_state=spl_rand) #rs
            # splitter = ShuffleSplit(n_splits=self.test_splitting_nature, test_size=self.test_split_ratio, random_state=random.randint(1, self.test_splitting_nature + 1))  # rs
            self.tst_nat = f'random-{split_num}'
        elif self.test_splitting_nature == 0:
            # k-Fold splitting
            split_num = num_fold
            split_num = split_num if (split_num>1 and split_num<=len(unique_pat_ids)) else 2
            splitter = KFold(n_splits=split_num, shuffle=True, random_state=spl_rand)  # kf
            # splitter = KFold(n_splits=num_fold)  # kf
            self.tst_nat = f'kfold-{split_num}'
        else:
            # # Leave-p-out
            split_num = -(self.test_splitting_nature)
            split_num = split_num if (split_num<=len(unique_pat_ids)) else 1
            splitter = LeavePOut(p=split_num)  # lpo
            # splitter = LeavePOut(p=int(len(unique_pat_ids)*self.test_split_ratio)) #lpo
            self.tst_nat = f'leave-{split_num}-out'

        util = Humachlab_Utility()
        ii = 0

        if self.patients_or_folds_done>0:##########
            print(splitter_b, type(splitter_b))
            splitter = splitter_b[0]
        ...
        for train_ids, test_ids in splitter.split(unique_pat_ids):
            ...
            # print('&&&&&--> ', unique_pat_ids, train_ids, test_ids)

            #Skipping already done test epochs
            if (ii+1)<=self.patients_or_folds_done:
                self.logger.info(f'@## Testing for patient at {ii+1} is already done for subjects: {test_ids}.')
                ii += 1
                continue

            # print('TS $$$$$$> ', ii, test_ids, train_ids)
            # continue
            ...
            util.start_timer()
            self.logger.info(f'#################################################################\n@## {ii+1} Testing STARTS for patient {test_ids}')
            ...
            best_model_index = 0
            all_trained_models, all_trained_models_scores, best_model_index = self.patient_specific_training(ii, train_ids, classifier_method, num_fold=(num_fold-1), is_train_will_all_data=is_train_will_all_data)
            model_scores, test_patient = [], {'exp_name': self.exp_name, 'test_patient': test_ids}
            self.test_patients.append(test_ids)
            ...
            best_extra_for_this_test = pd.DataFrame()
            best_label_for_this_test = []
            best_preds_for_this_test = []
            for i in range(len(all_trained_models)):
                ...
                if i==best_model_index:
                    ...
                    best_extra_for_this_test = pd.concat([best_extra_for_this_test, extra])
                    best_label_for_this_test.extend(y_test)
                    best_preds_for_this_test.extend(y_pred)

                X_test, y_test = None, None

            # Save best prediction result
            self.save_individual_model_prediction_targets(classifier_method, f'best_test-{ii}', best_extra_for_this_test, best_label_for_this_test, best_preds_for_this_test)
            ...
            self.save_and_show_model_scores(all_models, all_best_model, all_models_training_scores, all_models_test_scores, all_best_test_scores, splitter)
    ...
    def patient_specific_training(self, test_iter, training_ids, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False):
        all_trained_models, all_trained_models_scores = [], []

        splitter = None
        spl_rand = random.randint(1, 1000)
        self.logger.info(f'Splitter random value: {spl_rand}')
        if self.is_validate_models:
            if self.validation_splitting_nature > 0:
                # Random splitting
                split_num = self.validation_splitting_nature
                # split_num = split_num if (split_num<=len(unique_pat_ids)) else len(unique_pat_ids)
                splitter = ShuffleSplit(n_splits=split_num, test_size=self.validation_split_ratio, random_state=spl_rand)  # rs
                # splitter = ShuffleSplit(n_splits=self.validation_splitting_nature, test_size=self.validation_split_ratio, random_state=random.randint(1, self.validation_splitting_nature + 1))  # rs
                self.val_nat = f'random-{split_num}'
            elif self.validation_splitting_nature == 0:
                # k-Fold splitting
                split_num = num_fold
                split_num = split_num if (split_num > 1 and split_num <= len(training_ids)) else 2
                splitter = KFold(n_splits=split_num, shuffle=True, random_state=spl_rand)  # kf
                # splitter = KFold(n_splits=num_fold)  # kf
                self.val_nat = f'k fold-{split_num}'
            else:
                # # Leave-p-out
                split_num = -(self.validation_splitting_nature)
                split_num = split_num if (split_num <= len(training_ids)) else 1
                splitter = LeavePOut(p=split_num)  # lpo
                # splitter = LeavePOut(p=int(len(training_ids)*self.validation_split_ratio)) #lpo
                self.val_nat = f'leave-{split_num}-out'
        else:
            self.logger.warning(f'Model is not going to be validated, this might cause problem and program crashes...')
            pass
        ...
        for train_ids, val_ids in splitter.split(training_ids):
            ...
            print('TR *******> ', i, val_ids, train_ids, is_train_will_all_data)
            if is_train_will_all_data:
                train_ids.extend(val_ids)
                train_ids.sort()
                if i==0:
                    self.val_nat = self.val_nat + f' | train with all validation and training'


            util.start_timer()
            self.logger.info(f'@** {test_iter+1},{i+1} Training and validation STARTS for patient {val_ids}')
    ...
    def cross_patient_training_and_testing(self, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False, randomness_of_samples=True):
        ...
        self.tst_nat = f'kfold-{num_fold}'
        # print(len(serial_indices_for_folds))

        unique_pat_ids = list(self.dataset[self.pat_id_col].unique())

        all_models, all_best_model, all_models_training_scores, all_models_test_scores, all_best_test_scores, all_extra, all_y_tests, all_y_preds, self.best_model_indices, _ = self.load_partial_models_scores_prediction_and_metadata()
        ...
        for i, test_ids in enumerate(serial_indices_for_folds):
            #Skipping already done test epochs
            print((ii+1), self.patients_or_folds_done)
            if (ii+1)<=self.patients_or_folds_done:
                self.logger.info(f'@## Testing for fold {ii+1} is already done.')
                ii += 1
                continue
            ...
            util.start_timer()
            self.logger.info(f'#################################################################\n@## {ii+1} Testing STARTS for data {test_ids[:3]+["..."]}')
            self.logger.info(f'Total testing:{len(test_ids)}, Total training & validation:{len(train_ids)}')
            ...
            all_trained_models, all_trained_models_scores, best_model_index = self.cross_patient_training(ii, dat_serials, classifier_method, num_fold=(num_fold-1), is_train_will_all_data=is_train_will_all_data)
            model_scores, test_patient = [], {'exp_name': self.exp_name, 'test_patient': test_ids}
            self.test_patients.append(test_ids)
            self.best_model_indices.append(best_model_index)
    ...
    def cross_patient_training(self, test_iter, dat_serials, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False):
        all_trained_models, all_trained_models_scores = [], []
        ...
        if self.is_validate_models:
            # k-Fold splitting
            split_num = num_fold
            split_num = split_num if (split_num > 1 and split_num < len(dat_serials)) else 2
            splitter = KFold(n_splits=split_num, shuffle=True, random_state=spl_rand)  # kf
            self.val_nat = f'random-{split_num}'
            ...
            #     # # Leave-p-out
            #     split_num = -(self.validation_splitting_nature)
            #     split_num = split_num if (split_num < len(dat_serials)) else 1
            #     splitter = LeavePOut(p=split_num)  # lpo
            #     # splitter = LeavePOut(p=int(len(training_ids)*self.validation_split_ratio)) #lpo
            #     self.val_nat = f'leave-{split_num}-out'
        else:
        ...
        for train_ids, val_ids in splitter.split(dat_serials):
            ...
            if is_train_will_all_data:
                ...
                if i==0:
                    self.val_nat = self.val_nat + f' | train with all validation and training'
            util.start_timer()
            self.logger.info(f'@** {test_iter+1},{i+1} Training and validation STARTS for data {val_ids[:3]+["..."]}')
            self.logger.info(f'Total validation:{len(val_ids)}, Total training:{len(train_ids)}')
...
# ### Apply Custom grid-search and calculate scores
    ...
    def _do_custom_grideSearch(self, classifier_method, parameters, train_ids, val_ids, scoring, cv, split_ratio):
        ...
        spl_rand = random.randint(1, 1000)
        self.logger.info(f'Splitter random value in custom GridSearch: {spl_rand}')
        param_grid = ParameterGrid(parameters)
        self.logger.info(f'Fitting {cv} folds for each of {len(param_grid)} candidates, totalling {cv*len(param_grid)} fits:')
        # random_splitter = ShuffleSplit(n_splits=self.validation_splitting_nature, test_size=self.validation_split_ratio, random_state=spl_rand)  # rs
        random_splitter = ShuffleSplit(n_splits=cv, test_size=split_ratio, random_state=spl_rand)  # rs
        i = 0
...
# ### Apply Randomized-search and calculate scores
    ...
    def run_model_randomizedSearch(self, classifier_method, params, train_ids, val_ids):
        ...
        # model = RandomizedSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, iid=False, n_jobs=None, verbose=2)
        # model = RandomizedSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, iid=False, n_jobs=50, verbose=2)
        # model = RandomizedSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, iid=False, n_jobs=mp.cpu_count(), verbose=2)
...
# ### Apply Baysian-search and calculate scores
    ...
    def run_model_baysianSearch(self, classifier_method, params, train_ids, val_ids):
        ...
        # model = BayesSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, n_iter=(self.cross_validation_rounds * 2), iid=False, n_jobs=None, verbose=2)
        # model = BayesSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, n_iter=(self.cross_validation_rounds * 2), iid=False, n_jobs=50, verbose=2)
        # model = BayesSearchCV(mods, parameters, scoring=scoring, cv=self.cross_validation_rounds, refit=refit, return_train_score=True, n_iter=(self.cross_validation_rounds * 2), iid=False, n_jobs=mp.cpu_count(), verbose=2)
...
# ### All models' implementation
    ...
    def get_ml_model_instances(self, classifier_method, parameters=None):
        ...
        # self.logger.info(f'For GPU based tasks. There are {tot_gpus} GPUs in the system and {tot_avl_gpus} are available. \nAvailable GPU IDs are: {avl_GPUIDs}')
...
#############################
    ...
    def save_and_show_model_scores(self, all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs, splitter):

        # ### Save model scores and details
        test_no = self.test_no

        PIK = f'{self.log_path}/splitter_{test_no}.sdat'
        self.dataManager.save_dictionary_to_file(splitter, PIK, 'Spliter', dump_level=1, force_save=True)

        PIK = f'{self.log_path}/all_models_{test_no}.dat'
        self.dataManager.save_dictionary_to_file(all_models, PIK, 'All Models', dump_level=2, force_save=True)

        PIK = f'{self.log_path}/bst_models_{test_no}.dat'
        self.dataManager.save_dictionary_to_file(bst_models, PIK, 'Best Models', dump_level=2, force_save=True)

        # ### Writing scores in text file
        PIK = f'{self.log_path}/tr_score_{test_no}.txt'
        self.dataManager.save_dictionary_to_file(all_training_scrs, PIK, 'Training Scores', dump_level=1, force_save=True)

        PIK = f'{self.log_path}/ts_score_{test_no}.txt'
        self.dataManager.save_dictionary_to_file(all_test_scrs, PIK, 'Testing Scores', dump_level=1, force_save=True)

        PIK = f'{self.log_path}/bst_score_{test_no}.txt'
        self.dataManager.save_dictionary_to_file(bst_test_srcs, PIK, 'Best Testing Scores', dump_level=1, force_save=True)
    ...
    def save_metadata_and_prediction_targets(self, classifier_method, pat_ids, all_extra, y_tests, y_preds):
        ...
        metadata_dict['analysis_type'] = f'Test= {self.tst_nat}, Validation/Training= {self.val_nat}'
        ...
        metadata_dict['feature_type'] = self.feature_type
        ...
        val_nature = self.val_nat #'leave-one-out' if self.test_splitting_nature == 0 else (f'random-{self.test_splitting_nature}' if self.test_splitting_nature > 0 else f'leave-{int(len(self.patients) * self.test_split_ratio)}-out')
        tst_nature = self.tst_nat #'leave-one-out' if self.validation_splitting_nature==0 else (f'random-{self.validation_splitting_nature}' if self.validation_splitting_nature>0 else f'leave-{int(len(self.patients)*self.validation_split_ratio)}-out')
        metadata_dict['validation_splitting_nature'] = val_nature
        metadata_dict['validation_split_ratio'] = self.validation_split_ratio
        metadata_dict['test_splitting_nature'] = tst_nature
        ...
        metadata_dict['test_patients'] = self.test_patients
        ...
        self.dataManager.save_dictionary_to_file(metadata_dict, PIK, 'Testing Metadata', dump_level=1, force_save=True)
        ...
        self.dataManager.save_dataframe_to_file(df, save_file, 'Prediction Result', force_save=True)
    ...
    def save_individual_model_prediction_targets(self, classifier_method, test_or_val_detail, extra, y_test, y_pred):
        ...
        self.dataManager.save_dataframe_to_file(extra, save_file, 'Prediction Result', force_save=True)
        return


    def load_partial_models_scores_prediction_and_metadata(self):
        all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs, all_splitter_b = [], [], [], [], [], []
        all_extra = []
        all_y_tests = []
        all_y_preds = []
        best_model_indices = []
        ...
        if self.patients_or_folds_done>0:
            ...
            PIK = f'{self.log_path}'
            exp_srl = PIK.split('_')[-1]
            exp_srl = exp_srl.replace('/', '')
            exp_srl = int(exp_srl)
            exp_list = [i for i in range(exp_srl, (exp_srl+1))]
            path_items = PIK.split('/')
            # path_items
            PIK = '/'.join(path_items[:-2]) + '/'
            models, bst_models, tr_scores, ts_scores, bst_scores, metadata, prdiction_data, splitter_b = self.dataManager.load_all_previously_saved_results(PIK, exp_list)
            # all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs = models[0], bst_models[0], tr_scores[0], ts_scores[0], bst_scores[0]
            all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs, all_splitter_b = models[0], bst_models[0], tr_scores[0], ts_scores[0], bst_scores[0], splitter_b[0]
            # print('CCCCCCC->', metadata, all_splitter_b, prdiction_data)
            best_model_indices = metadata[0]['best_model_index']
            all_extra = []
            all_y_tests = []
            all_y_preds = []
            test_srl_nos = prdiction_data[0]['test_number'].unique() # list((prdiction_data['test_number'].unique())).sort()
            print(test_srl_nos, type(test_srl_nos))
            for tno in test_srl_nos:
                tmp_prdiction_data = prdiction_data[0][prdiction_data[0]['test_number']==tno]
                print(tmp_prdiction_data.columns.values.tolist())
                # all_extra.append( tmp_prdiction_data[~tmp_prdiction_data[self.class_name, self.pred_col]] )
                # all_extra.append( tmp_prdiction_data.loc[:, (tmp_prdiction_data.columns in [self.class_name, self.pred_col]) ])
                all_y_tests.append( tmp_prdiction_data[self.class_name].values.tolist() )
                all_y_preds.append( tmp_prdiction_data[self.pred_col].values.tolist() )
                all_extra.append( tmp_prdiction_data.drop([self.class_name, self.pred_col], axis=1) )
        ...
        return all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs, all_extra, all_y_tests, all_y_preds, best_model_indices, all_splitter_b
