...
######################################
    ...
    def set_or_load_all_data(self, path='./', feature_subdirectories='single_channel_feafures', replace_transitions=0, drop_nan=False, all_channels_columnwise=False, dataframe=None, dat_file_name=''):
        ...
        if self.dataset.shape[0]>0:
            ...
            self.dataset = self.dataManager.fill_or_remove_nan_value_for_all_channels(self.dataset, patients, channels, drop_nan)
            # Do post-processing for row-wise or column-wise data rearrangement
            if all_channels_columnwise:
                self.dataset = self.dataManager.create_columnwise_features_from_dataset(self.dataset, channels)
...
############################################################################
    ...
    # #########################################################################
    # Not patient specific analysis

    def apply_rf_classifier(clf_df, fold_range, comm_cols, class_name, col_wise=False):
        ts_dp_serial = []
        all_preds = []
        all_feat_importance = pd.DataFrame()
        all_feat_importance[f'features'] = [f for f in clf_df.columns.values.tolist() if f not in comm_cols]

        for i, v in enumerate(fold_range):
            si = (fold_range[i - 1] + 1) if i > 0 else 0
            ei = len(clf_df) if (i == len(fold_range) - 1) else (fold_range[i] + 1)
            print(si, ei)
            ts_dat = clf_df.iloc[si:ei, :]
            ind = clf_df.index.isin(ts_dat.index.tolist())
            tr_dat = clf_df[~ind]
            print(f'tr_shape-{tr_dat.shape} ts_shape-{ts_dat.shape} tot_row-{ts_dat.shape[0] + tr_dat.shape[0]}')
            print(
                f'tr_sz-{(tr_dat[class_name] == 1).sum()} tr_nsz-{(tr_dat[class_name] == 0).sum()} tot_row-{(tr_dat[class_name] == 1).sum() + (tr_dat[class_name] == 0).sum()}')
            print(
                f'ts_sz-{(ts_dat[class_name] == 1).sum()} ts_nsz-{(ts_dat[class_name] == 0).sum()} tot_row-{(ts_dat[class_name] == 1).sum() + (ts_dat[class_name] == 0).sum()}')
            ...
            if not col_wise:
                ...
                sz_col_indx = 4
            X_train, X_test, y_train, y_test = tr_dat.iloc[:, (sz_col_indx + 1):], ts_dat.iloc[:,
                                                                                   (sz_col_indx + 1):], tr_dat.iloc[:,
                                                                                                        sz_col_indx], ts_dat.iloc[
                                                                                                                      :,
                                                                                                                      sz_col_indx]
            ts_dp_serial.extend(ts_dat.iloc[:, 0].values.tolist())
            ...
            # Feature importance
            all_feat_importance[f'fold-{i}'] = clf.feature_importances_

        print(ts_dp_serial)
        print(all_preds)
        print(len(ts_dp_serial))
        print(len(all_preds))

        d = {comm_cols[0]: ts_dp_serial, pred_col: all_preds}
        df1 = pd.DataFrame(d)
        df1.sort_values(by=[comm_cols[0]], inplace=True)

        score_df = clf_df.copy()
        score_df = score_df[comm_cols]
        score_df.sort_values(by=[comm_cols[0]], inplace=True)
        score_df[pred_col] = df1[pred_col].copy()

        # Feature importance
        all_feat_importance['importance'] = (all_feat_importance.mean(axis=1)).values.tolist()

        return score_df, all_feat_importance
