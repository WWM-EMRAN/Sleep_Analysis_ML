...
# ### Necessary parameter setup for data management and training-testing
    ...
    def do_classification(self, classification_type, clf_meth, num_fold):
        ...
        if classification_type == 1:
            self.patient_specific_testing(classifier_method=clf_meth, num_fold=num_fold)
    ...
    def patient_specific_testing(self, classifier_method=ML_Classifiers.RF, num_fold=5):
        self.is_patient_spesific_analysis = True
        self.exp_type = f'patient-wise training and testing'
        ...
        if self.test_splitting_nature > 0:
            # Random splitting
            split_num = self.test_splitting_nature if (self.test_splitting_nature<=len(unique_pat_ids)) else 5
            ...
            # k-Fold splitting
            split_num = num_fold if (num_fold<=len(unique_pat_ids)) else 5
        ...
        for train_ids, test_ids in splitter.split(unique_pat_ids):
            ...
            best_model_index = 0
            all_trained_models, all_trained_models_scores, best_model_index = self.patient_specific_training(ii, train_ids, classifier_method, num_fold=num_fold)
    ...
    def patient_specific_training(self, test_iter, training_ids, classifier_method=ML_Classifiers.RF, num_fold=5):
        ...
        if self.is_validate_models:
            if self.validation_splitting_nature > 0:
                # Random splitting
                split_num = self.validation_splitting_nature if (self.validation_splitting_nature <= len(training_ids)) else 5
                ...
                # k-Fold splitting
                split_num = num_fold if (num_fold <= len(training_ids)) else 5
                splitter = KFold(n_splits=split_num, shuffle=True, random_state=random.randint(1, 1000))  # kf
                # splitter = KFold(n_splits=num_fold)  # kf
            else:
                # # Leave-p-out
