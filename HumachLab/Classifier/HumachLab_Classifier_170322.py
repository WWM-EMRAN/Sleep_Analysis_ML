...
# ### Necessary parameter setup for data management and training-testing
    ...
    def do_preprocessing(self, preprocessing_type, prep_feature_type=[]):
        ...
        # Apply domain based feature selection
        if preprocessing_type==1:
            # setattr(self, name, value)  # equivalent to: self.varname= 'something'
            self.dataset = self.preprocessor.get_selected_features_from_dataframe(self.dataset, self.class_name, prep_feature_type=prep_feature_type)
    ...
    def patient_specific_testing(self, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False):
        ...
        all_models, all_best_model, all_models_training_scores, all_models_test_scores, all_best_test_scores, all_extra, all_y_tests, all_y_preds, self.best_model_indices = self.load_partial_models_scores_prediction_and_metadata()
        ...
        if self.test_splitting_nature > 0:
            ...
            splitter = ShuffleSplit(n_splits=split_num, test_size=self.test_split_ratio, random_state=random.randint(1, 1000)) #rs
            ...
            splitter = KFold(n_splits=split_num, shuffle=True, random_state=random.randint(1, 1000))  # kf
        ...
        for train_ids, test_ids in splitter.split(unique_pat_ids):
            ...
            if (ii+1)<=self.patients_or_folds_done:
                self.logger.info(f'@## Testing for patient at {ii} is already done.')
                ii += 1
                continue
    ...
    def patient_specific_training(self, test_iter, training_ids, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False):
        ...
        if self.is_validate_models:
            if self.validation_splitting_nature > 0:
                ...
                # split_num = split_num if (split_num<=len(unique_pat_ids)) else len(unique_pat_ids)
                splitter = ShuffleSplit(n_splits=split_num, test_size=self.validation_split_ratio, random_state=random.randint(1, 1000))  # rs
    ...
    def cross_patient_training_and_testing(self, classifier_method=ML_Classifiers.RF, num_fold=5, is_train_will_all_data=False, randomness_of_samples=True):
        ...
        for i, test_ids in enumerate(serial_indices_for_folds):
            ...
            if (ii+1)<=self.patients_or_folds_done:
                self.logger.info(f'@## Testing for fold {ii} is already done.')
                ii += 1
                continue
...
#############################
    ...
    def load_partial_models_scores_prediction_and_metadata(self):
        all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs = [], [], [], [], []
        all_extra = []
        all_y_tests = []
        all_y_preds = []
        best_model_indices = []
        ...
        if self.patients_or_folds_done>0:
            ...
            models, bst_models, tr_scores, ts_scores, bst_scores, metadata, prdiction_data = self.dataManager.load_all_previously_saved_results(PIK, exp_list)
        ...
        return all_models, bst_models, all_training_scrs, all_test_scrs, bst_test_srcs, all_extra, all_y_tests, all_y_preds, best_model_indices
