...
class HumachLab_CHBMIT_Classifier:
    ...
    def __init__(self, log_path='./', should_use_params=False, cross_validation_rounds=5, exp_name='test', save_no='01', test_split_ratio=0.30, class_name='class', random_training=0):
    ...
    def load_all_data(self, path, feature_subdirectories, class_name, channels=[], patients=[], records=[],
                      all_channels_columnwise=False):
        # patient = ':02d'.format(patient)
        df = pd.DataFrame()
        path2 = os.walk(path)
        exclude_cols = ['pat_id', 'rec_id', 'ch_id', 'ch_name', class_name]
        ...
        for root, directories, files in path2:
            ...
            for directory in directories:
                ...
                if all_channels_columnwise:
                    ...
                    ex_cols.remove('ch_id')
                    ex_cols.remove('ch_name')
                    tdf.drop(['ch_id', 'ch_name'], axis=1, inplace=True)
                    ...
                    ### Check for the common columns already added and rename feature columns
    ...
    def patient_specific_training(self, data, method='rf'):
        ...
        for i in index_list:
            ...
            model, model_scores, model_scr = None, {'exp_name': self.exp_name, 'validation_patient': pat_id}, None
