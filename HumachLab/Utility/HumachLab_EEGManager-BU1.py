...
class HumachLab_EEGManager:
    ...
    def __init__(self, logger, data_directory='./', metadata_path='./', removable_channel_list=[], is_remove_duplicate_channels=False):
        ...
        if not (os.path.exists(self.data_directory)):
            ...
            self.data_directory = os.path.abspath('Classifier/')
    ...
    def _get_record_info_for_patient(self, patient_no, record_no):
        ...
        for p in list_of_patients:
            ...
            print(f'hiiiii: {p}={list_of_record}')

            patient = self._generate_records_for_single_patient(p, list_of_record, xl_data)
    ...
    def _generate_records_for_single_patient(self, patient_no, rec_list, xl_data):
        seizure_indicators = ['Sz'] #['Sz', 'IIC']
        rec_nums = []
        records = []
        sz_times = []
        sz_durations = []
        ...
        for i, rec in enumerate(rec_list):
            ...
            num_active_cols_in_sheet = 4
            record_df = (pd.read_excel(xl_data, record_sheet)).iloc[:, :num_active_cols_in_sheet]

            _, _ = self._generate_info_for_single_record(rec, record_df)
            if i==0:
                pass


        # Metadata management
        first_rec_indx = rec_list[0]
        sheets = xl_data.sheet_names
        record_sheet = f'cEEG-{first_rec_indx:>03} Annotations'
        record_df = pd.read_excel(xl_data, record_sheet)
        # patient_df = patient_df.dropna()
        record_sz_df = record_df[record_df['Tag'].str.contains('|'.join(seizure_indicators))]

        if (record_sz_df.shape)[0]>0:
            tag_name = list(record_sz_df['Tag'])
            rec_time = list(record_sz_df['Recording Time'])
            tg_st = -1
            tg_en = -1
            for i, tg in enumerate(tag_name):
                if tg.find('End')<0:
                    tg_st = i
                    tg_en = -1
                else:
                    tg_en = i
                    if tg_st>=0 and tg_en>=0:
                        # sz_times.append( (np.round(rec_time[tg_st]), np.round(rec_time[tg_en])) )
                        # sz_durations.append( (np.round(rec_time[tg_en] - rec_time[tg_st])) )
                        sz_times.append( (np.round(rec_time[tg_st]), np.round(rec_time[tg_en])) )
                        sz_durations.append( (int(rec_time[tg_en] - rec_time[tg_st])) )

                        # self.logger.info(f'{i}: {sz_durations}')
                    tg_st = -1
                    tg_en = -1

        # File path
        patient = Patient(patient_no)
        patients_path = f'{self.data_directory}/cEEG-{patient_no:>03}/'
        self.logger.info(f'Folder or patient: {patient_no} = {patients_path}')

        # Folder read
        path = os.walk(patients_path)

        file_sl = 0
        for root, directories, files in path:
            for file in files:
                self.logger.info(f'File or record: {file}')
                ff = file.split('.')
                file_name = ff[0]
                file_type = ff[-1]
                rec_no = int( ((file_name.split('-'))[1]).split(' ')[0] )
                if file_type != 'edf':# or (rec_no not in r_no):
                    continue

                self.logger.info(f'Here comes3...{rec_no}')
                rec_nums.append(rec_no)
                rec_path = f'{patients_path}{file}'
                record = Record(rec_no)

                edf_data = mne.io.read_raw_edf(rec_path)
                edf_dict = edf_data.__dict__
                edf_dict_info = edf_dict['info']
                if file_sl ==0:
                    patient.sam_rate = int(edf_dict_info['sfreq'])
                ...
                record.file_path = rec_path
                record.channel_id = 0  # position of channel set in patient info 'channels'
                raw_extra = edf_dict['_raw_extras'][0]
                record.record_times = (0, int(raw_extra['nsamples']/patient.sam_rate))  # '12:04:35' converted to 43259 sec
                record.record_duration = int(raw_extra['nsamples']/patient.sam_rate)  # sec
                record.num_seizures = len(sz_durations)
                record.seizure_times = sz_times
                record.seizure_durations = sz_durations
        ...
        self.logger.info(f'Records in {patient_no}: {rec_nums} = {len(records)}')
        patient.num_records = file_sl
        patient.record_numbers = rec_nums
        patient.records = records
        patient.num_seizure_records = rec_nums
        patient.seizure_record_numbers = records
        return patient


    def _generate_info_for_single_record(self, rec, record_df):
        seizure_indicators = ['Sz'] #['Sz', 'IIC']
        ...
        if (record_df.shape)[0]>0:
            sec_date = list(record_df[''])
            sec_time = list(record_df['Recording Time'])
            tag_names = list(record_df['Tag'])
            sz_indices = []

            for sz_tag in (seizure_indicators):
                for i, tag in enumerate(tag_names):
                    if ((i%2==0) and (f'{sz_tag} Start' in tag)):
                        sz_indices.append(i)
                    if ((i%2!=0) and (f'{sz_tag} End' in tag)):
                        sz_indices.append(i)
            ...
            for sz_tag in (seizure_indicators):
                lst = []
                for i, tag in enumerate(tag_names):
                    if ((i%2==0) and (f'{sz_tag} Start' in tag)) or ((i%2!=0) and (f'{sz_tag} End' in tag)):
                        lst.append(i)
                lst = [i for i, tag in enumerate(tag_names) if sz_tag in tag]
                sz_indices += lst


        # Reading record info from edf
